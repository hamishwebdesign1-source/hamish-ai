...
| The apex domain (`hamishai.org`) 308-redirects to `www.hamishai.org` at the hosting layer — `metadataBase`, `sitemap.ts`, `robots.ts`'s sitemap/host, and 5 structured-data files all hardcoded the apex, sending every canonical tag, sitemap entry, and schema URL through a redirect hop | Live `fetch()` against both hosts (apex: 308 on every path; www: clean 200 on every path) | **Fixed** across every crawler-facing file — email bodies and internal app links deliberately left alone (no crawler-facing impact) |
| 2 more real empty `alt=""` on meaningful content images: the 6 AI Solutions illustrations, and the case-study `Testimonial` component's signature image | Sitewide grep for `alt=""` | **Fixed** — both use each item's own real `name` field |
| 2 case-study meta descriptions (`the-gannet` 170ch, `lomond-and-grey` 167ch) past Google's truncation point — the underlying field is also real, visible hero-subtitle copy, not just a meta tag | Measured every case study's real `overview` length | **Fixed** — added an optional `metaDescription` field, condensed to the same real facts, used only for the meta tag; hero copy on both pages confirmed untouched |
| Confirmed clean (no fix needed, genuinely audited): no broken internal links sitewide, no duplicate `<title>` tags, all static meta descriptions within 70-160 chars, correct single-`priority`-image usage on the homepage (no LCP risk), FAQPage schema present on every page with a visible FAQ accordion | Source-wide route-matching script, title/description length scripts, `priority` grep, `FaqJsonLd` usage grep | No action needed |
| The real 404 page's 3 "helpful destinations" still pointed at the archived Edinburgh consultancy pages (Services, Portfolio, Book a consultation) — missed by every other homepage-swap cleanup | Live check on a real broken URL (confirmed real 404 status code, and the stale destination copy) | **Fixed** — now leads with the Platform's own real anchors (`/#how-it-works`, `/#pricing`), Portfolio kept as the one genuinely evergreen destination |
| `/embed-demo` (real, rendered content, not an API/redirect route) used robots.txt `disallow` instead of `noindex` — inconsistent with `robots.ts`'s own documented reasoning for why `/demo` and `/concepts` deliberately use noindex instead | Read `robots.ts`'s own comment explaining the noindex-vs-disallow distinction; confirmed `/embed-demo` sat in the disallow list anyway | **Fixed** — moved to `noindex` on the page itself, dropped from the disallow list, matching `/demo`/`/concepts` exactly |
| No `apple-touch-icon` anywhere — only `icon.svg`, which iOS Safari's "Add to Home Screen" doesn't fall back to | Live `<link rel>` dump on `/` — no `apple-touch-icon` present | **Fixed** — `apple-icon.tsx` (Next's file-convention icon generation, same mechanism as `opengraph-image.tsx`) reproduces `icon.svg`'s own real shapes/colours at 180x180, no new artwork |
| No web app manifest anywhere (no `manifest.ts`, no static file) — Android/Chrome's own "Add to Home Screen" equivalent gap | Confirmed no manifest file exists; live-fetched the generated manifest and confirmed `<link rel="manifest">` after the fix | **Fixed** — `manifest.ts`, every value real (root layout's own name/description, `icon.svg`'s own real background colour and icon) |
| No `<meta name="theme-color">` anywhere, and manifest.ts's own colour fields (just added) used the Home/Agency hero's dark tone rather than the site's actual predominant light background | Live check before/after; OKLCH → sRGB conversion of the real `--background` token, independently confirmed against `og-image.tsx`'s own `BRAND.paper` constant | **Fixed** — added `theme-color` via the current `viewport` export (checked Next's own bundled docs first, per AGENTS.md), corrected manifest.ts's colours to the same real `#f4f7fb` |
| The homepage OG-image highlight bug (fixed earlier as a one-off) turned out to be a real, recurring bug class — `/book` and `/portfolio`'s own opengraph-image.tsx had the identical issue (highlight text not matching title's own trailing punctuation), both rendering entirely unhighlighted | Audited every `(site)` opengraph-image.tsx's title/highlight pairing; confirmed both live before fixing | **Fixed at the root** — `og-image.tsx`'s own highlight matching now strips trailing punctuation before comparing, instead of patching each page individually; homepage's OG title also restored to its real, fully-punctuated form now that the underlying bug is gone |
| Confirmed clean (no fix needed, genuinely audited): no heading-level skips on any of 14 real pages checked (fresh DOM crawl, not source-only), every FAQPage schema question matches its own page's visible accordion text exactly, breadcrumb coverage is correctly scoped to the one genuinely nested route, no raw `<img>` tags bypassing `next/image` anywhere in `(site)` | Live DOM checks (heading-level script, jsdom-based FAQ/DOM comparison), sitewide grep | No action needed |
| 2 `Service` schemas (`/website-audit`, `/services`'s 4 packages) had no `url` field — every other entity schema on the site (Organization, Person, Product) includes one | Compared every schema type's field set sitewide | **Fixed** — both now carry their real, genuine canonical URL |
| Confirmed clean (no fix needed): trailing-slash handling (`/services/` correctly 308s to `/services`), sitemap.xml serves the correct `application/xml` content-type, Twitter Card is already `summary_large_image`, Brotli compression is active, no real Twitter/X handle exists anywhere to add as `twitter:site` (correctly not fabricated) | Live `curl`/fetch checks | No action needed |
| Only `Strict-Transport-Security` was set on every response (Vercel's own default) — `X-Content-Type-Options` and `Referrer-Policy` were both absent, checked by Lighthouse's Best Practices score | Live `curl -I` header dump | **Fixed** — both added via `next.config.ts` `headers()`, sitewide; deliberately did not add `Permissions-Policy` (needs a Studio camera/mic audit first) or CSP (needs an exhaustive external-source allowlist first) in the same pass |
| All 21 `/concepts` pages (real, personalised cold-outreach tools built for real prospects) showed the root layout's own default title in the browser tab, not the business the page impersonates — confirmed live, real gap since each page is a client component and can't export its own `metadata` | Live check across 5 different concept pages before and after | **Fixed** — one real, lightweight `layout.tsx` per folder providing only a real `title`, sourced from the actual `prospects.concept_slug` DB column (18 of 21) or each page's own visible hero copy (the 3 with no linked prospects row) |
| Same 21 `/concepts` pages also inherited the root layout's Platform-focused meta description, not anything about the business concept shown | Live check before/after | **Fixed** — same 21 files, description wording grounded in real, already-visible "Concept by Hamish AI for {Business}" copy several pages already show; `gowf` worded distinctly to match its own real "fictional test project" framing, not the other 20's "real prospect" phrasing |
| Demo pages (`/demo/*`, all 5) confirmed clean — already server components with real, distinct per-page titles | Sitewide scan for `"use client"` page.tsx files with no local metadata (initially scoped to public marketing routes only) | No action needed |
| A follow-up, truly sitewide (not scope-limited) rerun of that same scan found 2 more real instances of the identical gap: `/studio-action/[token]` (the weekly digest email's one-click action-confirmation page) and `/portal/login` (had no layout-level fallback at all) | Live check on both before/after | **Fixed** — a real, honest, generic title on each (deliberately brand-neutral on `/portal/login`, since one URL serves every tenant's own white-labeled portal) |
| Extending that same sweep to `/admin` (Hamish's own single-operator internal tool): `admin/layout.tsx` had no metadata fallback either — 0 of 19 real pages under `admin/(authed)` set their own title | Live check on `/admin/login` before/after (dev server, JS-exec `document.title`) | **Fixed** — one real fallback title (`Admin \| Hamish AI`) on `admin/layout.tsx`, same pattern as `/portal`; lower stakes than `/portal` (internal-only, not tenant/client-facing) but the same real gap |
| The identical gap on `/studio` itself — the actual paid product, not an internal tool. All 17 real pages under `studio/(authed)/*` had no metadata and the shared layout provided no fallback, so every section (Prospects, Clients, Analytics, Billing...) showed the same generic root title, indistinguishable from each other despite agencies genuinely tabbing between them | Live check on `https://www.hamishai.org/studio/analytics` via Claude-in-Chrome's real authenticated session, before the fix | **Fixed** — unlike `/admin`/`/portal`, every Studio page is a real server component, so each got its own real, distinct title directly (`<Page> \| Studio`) rather than one shared fallback; `website-builder/[id]` and `website-builder/guides/[tool]` get a real per-instance title via `generateMetadata` (the client's actual business name; the real AI tool name) instead of a static one. Live re-checked on production post-deploy: `/studio` tab title now reads `Command Centre \| Studio`. |
| Not a metadata gap, but reported live via screenshot, twice: the Command Centre's "Getting set up" card, stacked full-width above "Your next best actions", pushed the tabbed sections below the fold for a still-onboarding org — a real on-page UX cost that works against the same "get found, get understood, get chosen" goal this audit exists for. First attempt collapsed it behind an accordion; reported live again that wasn't the actual ask (hiding contents, not the real fix) — reverted, and the two cards now sit side by side (`lg:grid-cols-2`) instead | Live re-check on production (`hamishai.org/studio`) via Claude-in-Chrome, on a genuinely visible/foregrounded tab (an earlier check on a backgrounded tab reported a false single-column layout — `document.visibilityState: "hidden"`, `innerWidth: 0` — and needs a fresh tab to trust geometry checks against): confirmed both cards share the same `top` offset with different `left` (side by side), and 0 `[data-slot="accordion-trigger"]` elements remain | **Fixed** — plain list restored in `onboarding-checklist.tsx`, placed beside `actionsRequiredContent` in a 2-column grid in `studio/(authed)/page.tsx` |
| Also reported live via screenshot: the Command Centre's 5-card stat row (Business Health + Prospects/Clients/Conversion/Pipeline) rendered at visibly uneven heights (152/136/120/136/136px) — "Business Health" plus its inline help icon didn't fit the column's text width on one line and wrapped, and even the plain cards weren't uniform among themselves (a one-word label sat shorter than a two-word one that also wrapped) | Live re-check on production via Claude-in-Chrome (same visible-tab caveat as above): all 5 cards measured an identical 152px | **Fixed, then superseded below** — every stat card already set `h-full` for exactly this; the row's own `items-start` was silently overriding it. Removed `items-start` so the grid's default stretch applies — correct as far as it went, but see the next row for the real fix the user actually wanted |
| Follow-up live feedback on the same row: Business Health was diagnosed (correctly) as the actual reason the row looked bad — its shape (ring + two lines of text) never matched its 4 siblings (icon + one number + one label), so every height fix was just managing that mismatch rather than removing it. Asked to move it to the top-right of the header, beside the greeting, with the remaining 4 cards free to size down | Live re-check on production via Claude-in-Chrome (fresh, confirmed-visible tab): `[class*="lg:grid-cols-4"]` grid containing "Prospects found" has exactly 4 children, none containing "Business Health"; a ring+score badge renders beside the "Good morning, ..." h1 | **Fixed** — new `command-centre-health-badge.tsx`, no card chrome, sits directly in the header row; stat grid dropped to 4 columns of genuinely uniform cards. Still respects the org's real Command Centre layout config (hiding "health" in Settings hides the header badge too) |
| Not a metadata gap: `studio-assistant-widget.tsx` (the floating AI chat trigger) sat bottom-left, directly against the left sidebar nav — reported live, moved to bottom-right | Live re-check on production via Claude-in-Chrome: button's bounding rect sits at the right edge (39px from `window.innerWidth`), not the left | **Fixed** — `bottom-6 left-6` → `bottom-6 right-6` on both the trigger and its open panel |
| A real, verified CSS bug, not a preference: the left sidebar nav (`studio-nav.tsx`) was already written as `position: sticky` (deliberately, per its own prior comment) but wasn't actually staying pinned on scroll — reported live ("I want the same [as the fixed assistant button]"). Root cause: `aurora-bg`'s own `overflow: hidden` (globals.css, clips the ambient background glow) sat on the very div the sidebar is a descendant of — an ancestor's `overflow: hidden` silently breaks `position: sticky` for everything inside it | Live re-check on production via Claude-in-Chrome on `/studio/prospects`: `getComputedStyle` confirmed `position: sticky` both before and after, but the *rendered* behavior changed — before the fix, a real scroll gesture (computer-tool scroll, not `window.scrollTo`, which silently no-ops on some tabs) moved the aside's `top` in lockstep with `scrollY` (off-screen at -901px); after the fix, the same scroll to `scrollY: 1000` left the aside pinned at `top: 32px`, matching its intended `top-8` offset | **Fixed** — split the decorative glow into its own absolutely-positioned sibling div (its own isolated `overflow: hidden`) instead of wrapping the real page content in it; same visual background, sidebar's containing-block chain to the real page scroller no longer passes through an `overflow: hidden` box |
| Reported live as "clunky": the Prospects panel's "Find prospects now" button sat visually orphaned between the "Your niche" card and the separate "Search now" card, belonging to neither, even though it runs on the niche settings directly above it (saves them first, same as the explicit "Save niche" button). The niche card and "Search now" also read as two more items in a list rather than two alternative paths (a saved, recurring niche vs. one immediate one-off search) | Live re-check on production via Claude-in-Chrome (fresh tab): walked the DOM to confirm the "Find prospects now" button and the "Your niche" heading share the same `[data-slot="card"]` ancestor, and a real "or" divider (two lines either side of the word) sits between that card and "Search now" | **Fixed** — button moved inside the niche card as a persistent footer (stays visible even when the niche fields collapse to their one-line summary); "or" divider added between the two cards |
| Mobile usability — real device/viewport pass, now done on the homepage plus `/services`, `/about`, `/portfolio` | Live DOM checks (horizontal overflow, WCAG 2.2 AA contrast ratio, tap-target size against the real AA 24px bar, not the AAA-only 44px one) via a real 375×812 viewport, each page | **Confirmed clean, all 4 pages**: zero horizontal overflow, zero contrast failures (78/52/48 real text elements checked on services/about/portfolio respectively, 0 failing each). One ambiguous finding on `/services`, not treated as a violation: `SiteFooter`'s plain-text nav links (shared across every page) measure under the 24px tap-target minimum by a mechanical width/height check, but WCAG 2.5.8 has an explicit exception for inline navigational text where spacing prevents mis-taps — the same shared component the homepage pass already found clean — so this is logged as a likely false positive, not silently dropped or fixed on a low-confidence read. Still open: real screenshots inside the homepage's Studio Tour become small/hard-to-read at mobile width (a real UX observation, out of this audit's technical-SEO scope to fix) |
