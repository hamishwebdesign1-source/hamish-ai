...
| Not a metadata gap, but reported live via screenshot: the Command Centre's "Getting set up" onboarding checklist stayed fully expanded on every visit for a still-onboarding org, adding a fixed ~180px above the fold regardless of how much was already done — a real on-page UX cost that works against the same "get found, get understood, get chosen" goal this audit exists for (a cluttered first screen is exactly what pushes a returning tenant's own attention off the page) | Live re-check on production (`hamishai.org/studio`) via Claude-in-Chrome: confirmed collapsed by default (`"Getting set up · 2 of 4 done"`), and confirmed the accordion actually opens/closes (`aria-expanded` toggling true/false, real item hrefs present in the panel) | **Fixed** — extracted into `onboarding-checklist.tsx`, collapsed by default behind a real progress summary, using the same Accordion primitive `help-faq-list.tsx` already established for this pattern in Studio |
