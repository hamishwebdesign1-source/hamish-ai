...
| Demo pages (`/demo/*`, all 5) confirmed clean — already server components with real, distinct per-page titles; no other client-component page outside `/concepts` was found with the same metadata gap | Sitewide scan for `"use client"` page.tsx files with no local metadata | No action needed |
