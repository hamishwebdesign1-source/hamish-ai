...
| Also reported live via screenshot: the Command Centre's 5-card stat row (Business Health + Prospects/Clients/Conversion/Pipeline) rendered at visibly uneven heights (152/136/120/136/136px) — "Business Health" plus its inline help icon didn't fit the column's text width on one line and wrapped, and even the plain cards weren't uniform among themselves (a one-word label sat shorter than a two-word one that also wrapped) | Live re-check on production via Claude-in-Chrome (same visible-tab caveat as above): all 5 cards now measure an identical 152px | **Fixed** — every stat card already set `h-full` for exactly this; the row's own `items-start` was silently overriding it. Removed `items-start` so the grid's default stretch applies |
