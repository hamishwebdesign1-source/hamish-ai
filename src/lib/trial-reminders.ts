...
// what Stripe does for free. The real, non-redundant gap: an agency on
// the free trial who never enters a card gets cut off from prospecting
// the moment trial_ends_at passes (discover-leads.ts's billingRequired
// gate) with zero warning today.
//
...
export async function sendTrialReminders(now = new Date()) {
  ...
  const { data: orgs, error } = await admin
    .from("organisations")
    .select("id, name, trial_ends_at, trial_reminder_7d_sent_at, trial_reminder_1d_sent_at")
  ...
  for (const org of orgs ?? []) {
    ...
    if (daysLeft <= 7 && daysLeft > 1 && !org.trial_reminder_7d_sent_at) {
      ...
      sent.push(org.id);
    } else if (daysLeft <= 1 && !org.trial_reminder_1d_sent_at) {
      await sendClientEmail(
        owner.email,
