...
// rather than decoration for its own sake, distinct from every other
// concept page's motif.
  ...
  return (
    <div className="overflow-hidden rounded-xl border border-[#4a3820] shadow-2xl">
      <div className="flex items-center gap-1.5 border-b border-[#4a3820] bg-[#150f08] px-3 py-2">
        ...
        <span className="size-2.5 rounded-full bg-[#d4a017]/60" />
        <span className="size-2.5 rounded-full bg-emerald-500/40" />
        <span className="ml-2 font-mono text-[10px] tracking-wide text-[#c4b394] uppercase">
          The Coffee Neuk — live
        </span>
      </div>
      <div ref={scrollRef} className="flex h-[360px] flex-col gap-3 overflow-y-auto bg-[#241b12] p-4">
        {messages.map((m, i) => (
          <div
            key={i}
            className={
              m.role === "user"
                ? "ml-auto max-w-[85%] rounded-2xl rounded-br-sm bg-[#d4a017] px-3 py-2 text-sm text-[#241b12]"
          ...
          <div className="flex flex-wrap gap-2 pt-1">
            {SUGGESTED.map((p) => (
              <button
                ...
                className="rounded-full border border-[#4a3820] px-3 py-1.5 text-xs text-[#c4b394] transition-colors hover:border-[#d4a017] hover:text-[#f8f0dd]"
              >
                {p}
              </button>
            ))}
      ...
      >
        <input
          ...
          aria-label="Message"
          disabled={loading}
          className="h-9 flex-1 rounded-md border border-[#4a3820] bg-[#241b12] px-3 text-sm text-[#f8f0dd] outline-none placeholder:text-[#7a6a4f] focus-visible:border-[#d4a017]"
        />
        <button
          type="submit"
          aria-label="Send"
          disabled={loading || !input.trim()}
          className="flex size-9 shrink-0 items-center justify-center rounded-md bg-[#d4a017] text-[#241b12] transition-colors hover:bg-[#e0b230] disabled:opacity-40"
        >
...
export default function CoffeeNeukConcept() {
  return (
    <div className="min-h-screen bg-[#f8f4e8] text-[#241b12]">
      <div className="flex flex-wrap items-center justify-center gap-x-2 gap-y-1 bg-[#241b12] px-4 py-1.5 text-center text-[11px] text-[#c4b394]">
        <span>
      ...
      {/* Hero */}
      <section className="relative isolate overflow-hidden bg-[#241b12] text-[#f8f0dd]">
        <Image
        ...
        />
        <div className="absolute inset-0 bg-gradient-to-t from-[#241b12] via-[#241b12]/90 to-[#241b12]/55" />
        ...
        <div className="relative mx-auto max-w-5xl px-6 pt-24 pb-20 md:pt-32 md:pb-24">
          <Reveal>
            <p className="font-mono text-xs tracking-[0.25em] text-[#e8c268] uppercase">Linlithgow · West Lothian</p>
          </Reveal>
          <Reveal delay={80}>
            <h1
      ...
      <section id="services" className="mx-auto max-w-5xl px-6 py-24">
        <Reveal>
          <p className="font-mono text-xs tracking-[0.2em] text-[#d4a017] uppercase">What people love</p>
        </Reveal>
        <div className="mt-8 grid gap-4 sm:grid-cols-2">
          {HIGHLIGHTS.map((h, i) => (
            <Reveal key={h.title} delay={i * 80}>
              <div className="group h-full rounded-xl border border-[#e8dcc0] bg-white p-7 transition-all duration-300 hover:-translate-y-1 hover:border-[#d4a017] hover:shadow-xl">
                <h.icon className="size-6 text-[#d4a017] transition-transform duration-300 group-hover:scale-110" />
      ...
      <section className="relative overflow-hidden bg-[#d4a017] px-6 py-24 text-[#241b12]">
        <Reveal>
          <p
      ...
      {/* AI assistant */}
      <section id="assistant" className="bg-[#241b12] px-6 py-24 text-[#f8f0dd]">
        <div className="mx-auto max-w-2xl">
          <Reveal>
            <p className="font-mono text-xs tracking-[0.2em] text-[#e8c268] uppercase">Live demo</p>
      ...
      <section className="mx-auto max-w-5xl px-6 py-24">
        <Reveal>
          <p className="font-mono text-xs tracking-[0.2em] text-[#d4a017] uppercase">Right now</p>
        </Reveal>
      ...
      {/* Closing */}
      <section className="border-t border-[#4a3820] bg-[#241b12] px-6 py-24 text-center text-[#f8f0dd]">
        <Reveal>
          ...
          <Link
            ...
            className="mt-8 inline-block text-sm text-[#e8c268] underline underline-offset-4 hover:text-[#f0d38f]"
          >
            hamishai.org
          </Link>
        </Reveal>
      ...
      <footer className="bg-[#241b12] px-6 pb-10 text-center text-[11px] text-[#8a7a5f]">
