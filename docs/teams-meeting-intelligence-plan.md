...
Status: **paused on cost, 2026-08-08** — not abandoned, just not worth the spend right now. Phase 1 (scheduling only, no AI) is fully built, typechecks/lints/builds clean, and the OAuth connect flow is proven live (see the finding below). What's blocking a real end-to-end test is a genuine Microsoft 365 Business Basic subscription (~£64/year after a free trial) for a licensed mailbox — Hamish's call was that's too much to commit to at the moment. Nothing further needed here until that changes; the free trial itself costs nothing to start whenever the timing's better, and everything built so far will keep working once a licensed account connects. Phases 2–5 are still just the plan below. Written the same way `leads-automation-plan.md` was — grounded in the actual current state of `/admin/leads` (research pipeline, sales kit, pipeline widgets all live as of High Impact #6–9) rather than a hypothetical greenfield build.
...
**Live-tested finding (2026-08-08), blocking Phase 1 fully working:** the OAuth connect flow itself works end-to-end — `GET /me` succeeds, the token carries all seven requested scopes including `Calendars.ReadWrite`. But `/me/calendarView` (and by extension `/me/events`, `/me/onlineMeetings` — everything Phase 1-3 actually need) returns a bare 401. Root cause, confirmed via the token's own identity claims: the Microsoft account connected (Hamish's personal `outlook.com`, added as a guest — `#EXT#` — to the Azure-auto-created "Default Directory" tenant) has `mail: null`. It's an identity for managing Azure resources, not a licensed Microsoft 365 mailbox — there's no Exchange Online or Teams behind it at all. This is exactly the "verify the M365 licence before relying on it" risk flagged in section 2/9 before any code was written, now confirmed as the actual blocker. **Fix is on Hamish**: set up a real Microsoft 365 subscription (Business Basic covers Exchange Online + Teams, has a free trial) — ideally on the `hamishai.org` domain (e.g. `hamish@hamishai.org`) rather than trying to license the personal-account guest identity — then reconnect at `/admin/ms-setup` signing in as that licensed account instead.
