...
**Pause point — not yet done:** `schema-content-scripts.sql` needs to be run in the Supabase SQL editor before script generation can do anything (confirmed via a live test: the chain currently no-ops silently and safely — `researchContentIdea` catches the missing-table error from `generateContentScripts` and just leaves the idea at `status: 'researched'`, exactly the "one bad downstream call shouldn't undo a good upstream result" degradation it was designed for). Once applied, re-run research on a real idea and confirm scripts + a video prompt appear on its detail page.
