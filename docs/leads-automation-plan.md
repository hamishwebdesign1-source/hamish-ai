...
8. **One "sales kit" call instead of six.** LinkedIn message, meeting agenda, proposal outline, and the follow-up email are structurally identical drafting tasks to the existing email/call-script generators. Consolidate into one Claude call with a multi-field tool schema (`submit_sales_kit`) that returns all of them at once, using the cached research JSON as context — one system prompt paid once instead of six times, generated once and cached, regenerated only on demand.
9. **Extend the dashboard widgets list** (Highest value, Hot opportunities, Ready for proposal, Waiting for customer, Follow-up today, Recently researched) — all pure SQL/JS filters over the fields #6–7 add. Zero LLM cost.

### Larger Features (real architecture, worth scoping separately before building)

10. **Automated weekly lead-discovery job**, replacing what currently happens by hand in a Claude chat session every week. A scheduled job runs targeted searches per category/area, applies the Quick-Win #6 research pass to each candidate, and inserts new rows — but **queues them for a fast human approval pass** ("New this week: 12 candidates, swipe to approve") rather than auto-committing. Business discovery has real judgment calls (duplicate detection, "is this actually a fit") that shouldn't run fully unattended, at least at first.
