...
| 2 | No lead → client conversion exists anywhere. Converting a real prospect into a client means retyping business name, email, and notes from scratch on a completely separate form. | Phase 5, step 5 | Add a "Convert to client" action on the lead detail page that pre-fills `/admin/clients`'s Add a Client form (or creates the row directly) from the prospect's existing data. | Small–Medium | Open — a narrower related gap (concept pages invisible from the client view) was fixed directly; full lead→client conversion is not |
