...
| # | Finding | Where it surfaced | Recommendation | Effort | Status |
|---|---|---|---|---|---|
| 1 | AI research (`ResearchLeadButton` and the deep research pipeline) hard-requires an existing `website` field — refuses to run at all without one. | Phase 5, step 2 and step 3 — hit identically by both the manual button and the automated pipeline | Add a "no website" research mode that works from business name, category, and the signal/notes text instead of failing outright — arguably the businesses without a site yet are exactly the ones HamishAI should be best at researching, not worst. | Medium | **Fixed** — `siteCheck` is `SiteCheck \| null` throughout; the prompt treats "no website" as the primary finding, not missing data; scoring treats it as the strongest version of the same signal a broken domain gets. One fix covers both the manual button and the automated pipeline (the latter calls the former directly). Verified live: real, honest research produced for a genuine no-website test lead, both via the button and the auto-triggered background job. |
| 2 | No lead → client conversion exists anywhere. Converting a real prospect into a client means retyping business name, email, and notes from scratch on a completely separate form. | Phase 5, step 5 | Add a "Convert to client" action on the lead detail page that pre-fills `/admin/clients`'s Add a Client form (or creates the row directly) from the prospect's existing data. | Small–Medium | **Fixed** — real `clients.source_lead_id` link, "Convert to client" pre-fills the existing Add a Client form (business name/email/website/notes/concept_slug), redirects to the new client, and the lead switches to an "already converted" state. Verified live end-to-end with a throwaway test lead. |
| 3 | The "Add a Client" form's `email` field is stored but never checked by portal auth (`getPortalMembership` only reads `client_members`) — an admin filling it in reasonably expects it to grant login access, and nothing tells them it doesn't. | Phase 5, step 5–6, confirmed by reading `portal-membership.ts` and `addClient()` side by side | Either wire `addClient()` to also insert a `client_members` row from the same email (removes the trap entirely, one extra insert), or remove the field from the create form and point straight at the real invite flow. | Small | **Fixed** — `addClient()` now creates the membership and sends the same notification the real invite flow does; field relabelled to state what it does |
| 4 | One email can only ever belong to one client's portal (`getPortalMembership` resolves to the oldest invite, silently). Inviting an email already used elsewhere just doesn't work, with no error. | Phase 5, step 8 — hit with my own test email, had to work around it with a `+alias` | At minimum, check for an existing membership on invite and surface a clear error ("this email already has portal access to \[other client\]") rather than silently creating a dead invite. | Small | **Fixed** — both `inviteClientMember` and `addClient` now check first and surface a clear error naming the other client; verified live by reproducing the exact collision |
| 5 | No self-service or AI-assisted concept-page generation exists — every concept page is a fully hand-authored React file, confirmed by the codebase's own comment. With ~19 pages now built this way, a lot of chat-widget/hero/reveal-animation boilerplate is copy-pasted per page rather than shared. | Phase 5 (using the real, only path to build `/concepts/gowf`) | Two separable improvements: (a) extract the repeated `ConceptChat` component and the chat-route system-prompt pattern into shared code so a new concept page needs a fraction of the current boilerplate; (b) the bigger, roadmap-level idea — an AI-assisted first draft of a concept page from a lead's research data, with a human doing final design/copy passes rather than starting from a blank file every time. | (a) Small · (b) Large | Open |
| 6 | A client has no link back to whichever lead (if any) originally became it, so a concept page linked while something was still a lead becomes invisible from the client's own admin view the moment it converts — found live, not predicted, when looking at the real Gowf client page. | Post-Phase-8, live admin use | Gave clients their own independent `concept_slug` (new column + dropdown on the client Overview, same pattern as the lead page) rather than solving full lead-to-client linking. | Small | **Fixed** |
