...
| Monthly, 1st at 09:30 UTC | `/api/cron/monthly-reports` | Generates and sends that month's dated health-and-activity report snapshot for every client. |
...
| Daily, 11:00 UTC | `/api/cron/trial-reminders` | Emails an agency on the free trial 3 days out, 1 day out, and the day their trial lapses, and warns any org that's crossed 80% of a real monthly Agency Platform usage limit (`usage-limits.ts`), once per type per month. |
