...
| Daily, 11:00 UTC | `/api/cron/trial-reminders` | Emails an agency on the free trial 3 days out, 1 day out, and the day their trial lapses; warns any org that's crossed 80% of a real monthly Agency Platform usage limit (`usage-limits.ts`), once per type per month; sends `sendAutonomousFollowUps()` (`autonomous-outreach.ts`) — the one email-sendable step of `lead-status.ts`'s cadence, for orgs that have opted in and configured a reply-to email; and runs `runAutoDraftHighScoreProspectsRule()` (`automation-rules.ts`) — drafts a sales kit for up to 5 researched, untouched, score-4+ prospects per opted-in org. |
